// This is the server.js file for the "RawMongoCrud" mini-project.
// What this file does: it builds the real Express app, connects the
// "/books" URLs to their routes, and starts a real server on a fixed
...
const booksRouter = require("./routes/books.routes"); // Get the real router that knows about /books
...
app.use("/books", booksRouter); // Send every request starting with /books to the books router
...
if (require.main === module) { // Only run the code below if this exact file was started directly (not imported)
  const PORT = process.env.PORT ?? 4131; // Use the PORT from .env, or 4131 if none is set
