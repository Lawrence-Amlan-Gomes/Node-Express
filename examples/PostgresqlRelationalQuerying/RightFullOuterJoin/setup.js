// This is the setup.js file for the "RightFullOuterJoin" mini-project.
// What this file does: run once by hand (npm run setup) — makes this
// mini-project's own schema, then customers/orders again, but this time
// customer_id is NULLABLE — a real "guest checkout" order can exist with
// no customer at all, which is exactly what FULL OUTER JOIN needs to prove.
...
async function main() { // Start the main function, and pause on each "await" until it finishes
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ${SCHEMA}.customers ( -- Make a new table called "customers", only if it is not there already
      id SERIAL PRIMARY KEY, -- The unique id an order MAY point back at
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ${SCHEMA}.orders ( -- Make a new table called "orders", only if it is not there already
      ...
      customer_id INTEGER REFERENCES ${SCHEMA}.customers(id), -- NOT "NOT NULL" this time — a real guest order can leave this empty
