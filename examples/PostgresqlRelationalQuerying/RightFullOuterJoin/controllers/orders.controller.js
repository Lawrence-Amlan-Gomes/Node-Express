// This is the orders.controller.js file for the "RightFullOuterJoin"
// mini-project. What this file does: create/list/delete-all for the real
// "orders" table — customerId is optional here, since a real guest order
// genuinely has no customer at all.
...
async function createOrder(req, res) { // Runs when someone sends POST /orders
  const { customerId, product } = req.body; // Pull the real values the client sent — customerId may be missing (a guest order)
  const result = await pool.query( // Send an INSERT command to the database and wait for the real answer
    ...
    [customerId ?? null, product] // customerId ?? null: if customerId was never sent, store a real database NULL instead of "undefined"
  );
