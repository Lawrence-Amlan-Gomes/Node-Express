// This is the setup.js file for the "InnerLeftJoin" mini-project.
// What this file does: run once by hand (npm run setup) — makes this
// mini-project's own schema, then the same real customers/orders shape as
// "PrimaryKeysForeignKeys", in its own separate, dedicated schema.
...
async function main() { // Start the main function, and pause on each "await" until it finishes
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ${SCHEMA}.customers ( -- Make a new table called "customers", only if it is not there already
      id SERIAL PRIMARY KEY, -- The unique id every order will point back at
