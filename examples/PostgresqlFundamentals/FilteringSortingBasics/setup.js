// This is the setup.js file for the "FilteringSortingBasics" mini-project.
// What this file does: same overall shape as setup.js in
// "PostgresConnectionAndTypes" (run once by hand, makes a schema, then a
// table) — the new part here is the "movies" table, made for real
...
async function main() { // Start the main function, and pause on each "await" until it finishes
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ${SCHEMA}.movies ( -- Make a new table called "movies", only if it is not there already
      id SERIAL PRIMARY KEY, -- Make an "id" column that counts up by itself and is the unique key for each row
      title TEXT NOT NULL, -- Make a "title" column for plain text that can never be left empty
      genre TEXT NOT NULL, -- Make a "genre" column for plain text that can never be left empty
      rating NUMERIC(3,1) NOT NULL, -- Make a "rating" column for a decimal number (like 8.7) that can never be left empty
      release_year INTEGER NOT NULL -- Make a "release_year" column for a whole number that can never be left empty
    ) -- This closing bracket ends the list of columns
  `); // This closing backtick ends the SQL command we are sending

  console.log(`Real schema "${SCHEMA}" and real table "movies" are ready.`); // Print a message so we can see it worked
