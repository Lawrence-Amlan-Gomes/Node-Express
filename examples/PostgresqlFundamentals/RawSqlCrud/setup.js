// This is the setup.js file for the "RawSqlCrud" mini-project.
// What this file does: same overall shape as setup.js in
// "PostgresConnectionAndTypes" (run once by hand, makes a schema, then a
// table) — the new part here is the "books" table, made for real CRUD.
...
async function main() { // Start the main function, and pause on each "await" until it finishes
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS ${SCHEMA}.books ( -- Make a new table called "books", only if it is not there already
      ...
      author TEXT NOT NULL, -- Make an "author" column for plain text that can never be left empty
      published_year INTEGER NOT NULL -- Make a "published_year" column for a whole number that can never be left empty
    ) -- This closing bracket ends the list of columns
  `); // This closing backtick ends the SQL command we are sending

  console.log(`Real schema "${SCHEMA}" and real table "books" are ready.`); // Print a message so we can see it worked
